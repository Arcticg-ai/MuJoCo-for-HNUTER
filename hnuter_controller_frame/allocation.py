...
class ActuatorAllocation:
    ...
    def inverse_nonlinear_mapping(self, W, state):
        ...
        Fz_front = Fz
        u2 = Fz_front/2 - (10/3)*Tx
        u5 = Fz_front/2 + (10/3)*Tx
    ...
    def allocate_actuators(self, f_c_body: np.ndarray, tau_c: np.ndarray, state: Dict[str, Any]) -> Tuple[float, float, float, float, float, float, float]:
        ...
        self.controller.T12 = F1
        self.controller.T34 = F2
        self.controller.T5 = F3
        self.controller.alpha1 = alpha1
        self.controller.alpha2 = alpha2
        self.controller.theta1 = theta1
        self.controller.theta2 = theta2

        # 存储控制输入向量
        self.controller.u = np.array([F1, F2, F3, alpha1, alpha2, theta1, theta2])
