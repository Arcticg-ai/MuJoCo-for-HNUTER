...
class HnuterController:
    def __init__(self, model_path: str = "scene.xml"):
        ...
        self.KR = np.array([5, 15, 5])   # 姿态增益
        self.Domega = np.array([1.5, 0.5, 2.0])  # 角速度阻尼
    ...
    def get_state(self) -> dict:
        ...
        try:
            ...
            if 'drone_pos' in self.sensor_ids:
                ...
                # 确保有足够的数据
                if self.model.sensor_dim[pos_sensor_id] >= 3:
            ...
            if 'drone_quat' in self.sensor_ids:
                ...
                # 确保有足够的数据
                if self.model.sensor_dim[quat_sensor_id] >= 4:
            ...
            if 'body_vel' in self.sensor_ids:
                ...
                # 确保有足够的数据
                if self.model.sensor_dim[vel_sensor_id] >= 3:
            ...
            if 'body_gyro' in self.sensor_ids:
                ...
                # 确保有足够的数据
                if self.model.sensor_dim[gyro_sensor_id] >= 3:
            ...
            if 'body_acc' in self.sensor_ids:
                ...
                # 确保有足够的数据
                if self.model.sensor_dim[accel_sensor_id] >= 3:
    ...
    def print_status(self):
        ...
        try:
            ...
            print(f"执行器状态: T12={self.T12:.2f}N, T34={self.T34:.2f}N, T5={self.T5:.2f}N, α1={math.degrees(self.alpha1):.1f}°, α2={math.degrees(self.alpha2):.1f}°, theta1={math.degrees(self.theta1):.1f}°, theta2={math.degrees(self.theta2):.1f}°")
            print("--------------------------------------------------")
        except Exception as e:
            print(f"状态打印失败: {e}")
