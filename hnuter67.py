...
class HnuterController:
    ...
    def inverse_nonlinear_mapping(self, W):
        ...
        # 求解角度 (这里是核心，alpha不被强行约束相等，而是自然解出)
    ...
    def update_trajectory(self, time: float):
        ...
        if time < 3.0:
            # 悬停阶段
            self.target_position = np.array([0.0, 0.0, 2.3])
        elif time < 8.0:
            # 前进阶段
            forward_distance = 5.5 * (time - 3.0) / 5.0  # 5秒内前进0.5米
            self.target_position = np.array([forward_distance, 0.0, 2.3])
        elif time < 15.0:
            # 后退阶段
            backward_distance = 5.5 * (15.0 - time) / 7.0  # 7秒内后退到原点
            self.target_position = np.array([backward_distance, 0.0, 2.3])
        else:
            # 最终悬停
            self.target_position = np.array([0.0, 0.0, 2.3])
